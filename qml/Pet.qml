...
Item {
    ...
    Connections {
        target: brainController
        function onUpdated() {
            if (!brainController || !petInstance.parent) return

            var muscles = brainController.muscles

            var left = brainController.leftMotor
            var right = brainController.rightMotor
            ...
            if (newX <= 0 || newX >= maxX || newY <= 0 || newY >= maxY) {
                if (brainController) {
                    ...
                    brainController.stimulateTouch()
                }
